...
function generateRandomTickerConfig(ticker) {
    ...
    switch (ticker) {
        case "TST":
            baseMidPrice = randomFloat(140, 160);
            ...
            break;
        case "RPP2":
            baseMidPrice = randomFloat(160, 180);
            ...
            break;
        case "BBB":
            baseMidPrice = randomFloat(25, 35);
        ...
        default: // For any new or unspecified tickers
            baseMidPrice = randomFloat(10, 200);
            baseVolatility = randomFloat(0.001, 0.1); // 0.1% to 2%
            baseSpread = randomFloat(0.05, 0.50);
...
function generateMarketMakerConfig(userId, name) {
    const marketConfig = {};
    const numTickers = randomInt(1, ALL_AVAILABLE_TICKERS.length); // Each bot trades 1 to all available tickers
    const shuffledTickers = [...ALL_AVAILABLE_TICKERS].sort(() => 0.5 - Math.random()); // Shuffle tickers

    for (let i = 0; i < numTickers; i++) {
        const ticker = shuffledTickers[i];
...
async function runMarketMakingCycle() {
    ...
    for (const marketMaker of marketMakers) {
        ...
        for (const ticker in marketMaker.marketConfig) {
            ...
            ordersPromises.push(addOrder(marketMaker.userId, ticker, 1, bidPrice, orderAmount));  // Place BUY order
            ordersPromises.push(addOrder(marketMaker.userId, ticker, -1, askPrice, orderAmount)); // Place SELL order
        }
        // Wait for all orders for the current market maker to be processed before moving to the next
